...
sealed interface SequentialTimerState {
    ...
    data class Running<T>(val items: List<T>, val currentItemIndex: Int, val timeLeft: Duration) :
        SequentialTimerState

    data class Paused<T>(val items: List<T>, val currentItemIndex: Int, val timeLeft: Duration) :
        SequentialTimerState
